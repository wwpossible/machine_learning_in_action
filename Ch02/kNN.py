...
###通过KNN进行分类
def classify(input, df, k):
    dataSize = len(df)  # dataframe的总行数
    ####计算欧式距离
    diff = tile(input, (dataSize, 1)) - df.values
    sqdiff = diff ** 2
    squareDist = sum(sqdiff, axis=1)  ###行向量分别相加，从而得到新的一个行向量
    dist = squareDist ** 0.5

    ##对距离进行排序
    sortedDistIndex = argsort(dist)##argsort()根据元素的值从小到大对元素进行排序，并返回index值

    classCount = {}
    for i in range(k):
        voteLabel = df.index[sortedDistIndex[i]]#voteLabel即为df的index->ABCDE
        ###对选取的K个样本所属的类别个数进行统计
        classCount[voteLabel] = classCount.get(voteLabel, 0) + 1
    ###选取出现的类别次数最多的类别
    maxCount = 0
    for key, value in classCount.items():#返回的是键值key和内容value
        if value > maxCount:
            maxCount = value
            classes = key
