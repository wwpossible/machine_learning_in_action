...
datingDataMat, datingLabels = kNN.file2matrix(r"C:\Python34\code\machinelearninginaction\Ch02\datingTestSet2.txt")
...
ax1.scatter(datingDataMat[:,0], datingDataMat[:,1], 10*np.array(datingLabels), 10*np.array(datingLabels))

ax2 = fig.add_subplot(312)#第二幅图
#散点图,X:每年获得的飞行常客里程数, Y:每周所消费的冰淇淋公升数，两个10*np.array(datingLabels)是scatter中的用法，用数字标识XY值的属性类别
ax2.scatter(datingDataMat[:,0], datingDataMat[:,2], 10*np.array(datingLabels), 10*np.array(datingLabels))

ax3 = fig.add_subplot(313)#第三幅图
#散点图,X:玩视频游戏所耗时间百分比, Y:每周所消费的冰淇淋公升数，两个10*np.array(datingLabels)是scatter中的用法，用数字标识XY值的属性类别
ax3.scatter(datingDataMat[:,1], datingDataMat[:,2], 10*np.array(datingLabels), 10*np.array(datingLabels))
